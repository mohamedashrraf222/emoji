...
class EmojiMatchZWJ(EmojiMatch):
    ...
    def __init__(self, match: EmojiMatch):
        ...
        i = match.start
        emoji_split = match.emoji.split(_ZWJ)
        # Precompute lengths for faster index calculation
        # and build all EmojiMatch in a tight loop, avoid attribute .emojis assignment overhead until the end
        for e in emoji_split:
            length_e = len(e)
            emojis.append(
                EmojiMatch(e, i, i + length_e, unicode_codes.EMOJI_DATA.get(e, None))
            )
            i += length_e + 1
        self.emojis: List[EmojiMatch] = emojis
    ...
    def join(self) -> str:
        ...
        """
        # Micro-optimization: explicit list comprehension is slightly faster
    ...
    def __repr__(self) -> str:
        # Cache class name to local variable for minor formatting speedup
        name = self.__class__.__name__
        # Inline join call for less attribute lookup
        joined = _ZWJ.join([e.emoji for e in self.emojis])
        return f"{name}({joined}, {self.start}:{self.end})"
